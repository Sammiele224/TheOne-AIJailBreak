...
async def _resolve_level_config_by_position(db: Session, level_id: int) -> LevelConfig:
    """Resolve a level config by gameplay level number."""

    statement = select(LevelConfig).where(LevelConfig.level_number == level_id)
    level_config = await db.execute(statement).scalar_one_or_none()
...
def _evaluate_response(
    *,
    ...
    if level_number == 2:
        return ToolVerifier(response=raw_response).evaluate()
...
async def submit_prompt(
    ...
    try:
        ...
        active_level = await db.execute(select(LevelConfig).where(LevelConfig.id == session.level_id)).scalar_one_or_none()
        ...
        await db.add(prompt_log)
        await db.add(verification_result)
        await db.flush()
        await db.commit()
        await db.refresh(session)
        await db.refresh(prompt_log)
        await db.refresh(verification_result)
